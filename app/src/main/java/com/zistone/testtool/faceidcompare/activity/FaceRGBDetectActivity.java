package com.zistone.testtool.faceidcompare.activity;
...
import com.zistone.testtool.FaceIdCompareActivity;
import com.zistone.testtool.R;
import com.zistone.testtool.faceidcompare.callback.CameraDataCallback;
import com.zistone.testtool.faceidcompare.callback.FaceDetectCallBack;
import com.zistone.testtool.faceidcompare.camera.AutoTexturePreviewView;
import com.zistone.testtool.faceidcompare.camera.CameraPreviewManager;
import com.zistone.testtool.faceidcompare.manager.FaceTrackManager;
...
import com.zistone.testtool.faceidcompare.utils.BitmapUtils;
import com.zistone.testtool.faceidcompare.utils.FaceOnDrawTexturViewUtil;
import com.zistone.testtool.faceidcompare.utils.FileUtils;
import com.zistone.testtool.faceidcompare.utils.ImageUtils;
import com.zistone.testtool.faceidcompare.view.FaceRoundView;
...
public class FaceRGBDetectActivity extends BaseActivity {
    private static final String TAG = "FaceRGBDetectActivity";
    ...
    private void faceDetect(byte[] data, int width, int height) {
        ...
        FaceTrackManager.getInstance().faceTrack(data, width, height, new FaceDetectCallBack() {
            ...
            public void onFaceDetectCallback(LivenessModel livenessModel) {
                if (livenessModel == null || livenessModel.getTrackFaceInfo() == null || livenessModel.getTrackFaceInfo().length == 0) {
                    clearTip();
    ...
    private void showDetectImage(byte[] rgb) {
        ...
        BDFaceImageInstance rgbInstance = new BDFaceImageInstance(rgb, mHeight, mWidth, BDFaceSDKCommon.BDFaceImageType.BDFACE_IMAGE_TYPE_YUV_NV21, SingleBaseConfig.getBaseConfig().getDetectDirection(), SingleBaseConfig.getBaseConfig().getMirrorRGB());
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                if (livess != null && livess.equals("livess")) {
                    Log.e(TAG, "检测耗时：" + String.valueOf(livenessModel.getRgbDetectDuration()));
    ...
    private void isInserLimit(final LivenessModel livenessModel) {
        ...
        FaceOnDrawTexturViewUtil.converttPointXY(pointXY, mPreviewView, livenessModel.getBdFaceImageInstance(), livenessModel.getFaceInfo().width);
        ...
        if (pointXY[0] - pointXY[2] / 2 < lfetLimitX || pointXY[0] + pointXY[2] / 2 > rightLimitX || pointXY[1] - pointXY[2] / 2 < topLimitY || pointXY[1] + pointXY[2] / 2 > bottomLimitY) {
            requestToInner = true;
        } else {
            requestToInner = false;
        }
